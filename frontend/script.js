...
window.changeLocation = changeLocation;// ============== EVENT HANDLERS ==============
          ...
          if (pageId === 'dashboard-page') {
              setTimeout(() => {
                  fetchData().then(data => {
                      ...
                      initPer5MinChart(data.per5Min); // BARU
                      initMap();
                  });
              }, 100);
          }
...
// Show parameter modal with data - UPDATED dengan threshold baru
...
// Handle pollutant tabs in education card and mini-tabs for 5-minute chart - UPDATED
function handlePollutantTabs() {
  const pollutantTabs = document.querySelectorAll('.pollutant-tab');
  
  pollutantTabs.forEach(tab => {
      tab.addEventListener('click', function() {
          // Cek apakah ini mini-tab atau tab normal
          const isMiniTab = this.classList.contains('mini-tab');
          
          if (isMiniTab) {
              // Untuk mini-tabs (parameter per 5 menit)
              ...
              // Refresh chart dengan parameter baru
              fetchPer5MinuteData().then(data => {
                  initPer5MinChart(data);
              });
          } else {
              // Untuk pollutant tabs di education card
              const pollutantTabs = document.querySelectorAll('.pollutant-tab:not(.mini-tab)');
...
function changeLocation(location) {
  if (locationCoordinates[location]) {
      ...
      per5MinCache.location = ''; // BARU - Reset per5Min cache juga
...
// Initialize chart per 5 menit - BARU
function initPer5MinChart(data) {
    const ctx = document.getElementById('per-5-min-chart');
    if (!ctx) return;
    
    const ctxContext = ctx.getContext('2d');
    
    if (per5MinChart) {
        per5MinChart.destroy();
    }
    ...
    // Check if we have data
    if (historyData.length === 0) {
        console.warn('⚠️ No 5-minute history data available for chart');
        ctxContext.clearRect(0, 0, ctx.width, ctx.height);
        ctxContext.font = '16px Arial';
        ctxContext.textAlign = 'center';
        ctxContext.fillText('Data histori per 5 menit belum tersedia', ctx.width / 2, ctx.height / 2);
        return;
    }
    ...
    const labels = historyData.map(item => formatTime(item.timestamp));
    const paramData = historyData.map(item => {
        // Handle parameter key properly
        if (selectedParameter === 'pm25') return item['pm2.5'] || 0;
        return item[selectedParameter] || 0;
    });
    
    // Create different colors based on parameter
    ...
    // Label pada sumbu Y sesuai parameter
    let yAxisLabel = 'Nilai';
    switch(selectedParameter) {
        case 'aqi': yAxisLabel = 'Nilai AQI'; break;
        case 'pm25': yAxisLabel = 'PM2.5 (µg/m³)'; break;
        case 'pm10': yAxisLabel = 'PM10 (µg/m³)'; break;
        case 'co': yAxisLabel = 'CO (µg/m³)'; break;
        case 'ozone': yAxisLabel = 'O₃ (µg/m³)'; break;
        case 'no2': yAxisLabel = 'NO₂ (µg/m³)'; break;
        case 'nh3': yAxisLabel = 'NH₃ (µg/m³)'; break;
    }
    
    per5MinChart = new Chart(ctxContext, {
        type: 'line',
        data: {
            labels: labels,
            datasets: [{
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            return `${selectedParameter.toUpperCase()}: ${value}${unit}`;
                        }
                    },
                    backgroundColor: 'rgba(255, 255, 255, 0.9)',
                    titleColor: '#333',
...
// Fetch data histori per 5 menit - BARU
async function fetchPer5MinuteData() {
    try {
        const endpoint = locationEndpoints[currentLocation];
        
        // Cek apakah data ada di cache dan masih valid
        const now = Date.now();
        const cacheTime = 2 * 60 * 1000; // Cache selama 2 menit
        
        if (per5MinCache.data.length > 0 && 
            now - per5MinCache.timestamp < cacheTime &&
            per5MinCache.location === currentLocation) {
            
            console.log(`📋 Using cached data for 5-minute history`);
            return per5MinCache.data;
        }
        
        // Jika tidak ada di cache, ambil dari API
        const url = `${BASE_URL}/${endpoint}/history?range=5m`;
        
        console.log(`📡 Fetching 5-minute history data from: ${url}`);
        ...
        const data = await response.json();
        console.log('✅ 5-minute history data received:', data);
        ...
        console.error('❌ Error fetching 5-minute history data:', error);
        return [];
    }
}
...
async function fetchData() {
    try {
        ...
        const [currentData, historyData, forecastData, per5MinData] = await Promise.all([
            fetchCurrentData(),
            fetchHistoryData(),
            fetchForecastData(),
            fetchPer5MinuteData() // BARU
        ]);
...
// Selected parameter for 5-minute chart - BARU
let selectedParameter = 'aqi';
...
// Cache untuk data per 5 menit - BARU
let per5MinCache = {
    data: [],
    timestamp: 0,
    location: ''
...
// Get parameter status based on value and parameter type
function getParameterStatus(param, value) {
    // Threshold values updated as per new requirements
    const thresholds = {
        'pm25': {
            good: 15.5,
            moderate: 55.4,
