...
const LOCATIONS = [
  { id: 'sensor001', name: "Jalan Radio", lat: -6.9947, lon: 107.6396 },
...
async function fetchPollutionForecast(lat, lon) {
  const url = `http://api.openweathermap.org/data/2.5/air_pollution/forecast?lat=${lat}&lon=${lon}&appid=${OPENWEATHER_API_KEY}`;
  const response = await axios.get(url);
  return response.data;
}

// Simpan data forecast beserta AQI ke Firestore
async function savePollutionDataToFirestore(sensorId, locationName, pollutionData) {
  ...
  try {
    ...
    const results = await Promise.all(
      LOCATIONS.map(async (loc) => {
        ...
        await savePollutionDataToFirestore(loc.id, loc.name, forecastData);
        return {
          id: loc.id,
          location: loc.name,
          status: 'success',
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({
        message: 'Forecasts saved from OpenWeather API',
        results,
